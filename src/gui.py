...
def ui_info():
    ...
    ui_spacer(1)
    st.markdown(
        "Source code can be found [here](https://github.com/yiqiao-yin/wyn-docquery)."
    )
...
def ui_model():
    ...
    st.selectbox(
        "embedding model", ["text-embedding-ada-002"], key="model_embed"
    )  # FOR FUTURE USE
...
def b_ask():
    ...
    #
    disabled = (not ss.get("api_key") and not ss.get("community_pct", 0)) or not ss.get(
        "index"
    )
    if c1.button(
        "get answer", disabled=disabled, type="primary", use_container_width=True
    ):
